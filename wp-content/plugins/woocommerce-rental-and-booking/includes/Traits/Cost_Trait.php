...
trait Cost_Trait
{
    ...
    public function calculate_rental_cost($productId, $inventoryId, $durations, $args = [])
    {
        ...
        $costByDays  = $prices['costByDays'];
        $costByHours = $prices['costByHours'];

        $durationTotal = $costByDays + $costByHours;
        $durationTotal = apply_filters('rnb_duration_total', $durationTotal, $durations, $productId, $inventoryId);
        ...
        if (empty($pre_payment_percentage) || $displays['instance_payment'] === 'closed') {
        ...
        return  apply_filters('rnb_rental_cost_details', [
            ...
            'instant_pay'     => $pre_payment_percentage,
    ...
    {
        $scheme = 0;
        $days   = intval($durations['flat_hours']) / 24;
