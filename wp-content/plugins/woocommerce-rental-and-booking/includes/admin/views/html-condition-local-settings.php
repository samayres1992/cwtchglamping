<div class="rnb-setting-fields rnb-condition-fields">
    ...
    <p class="form-field">
        ...
        <select multiple="multiple" class="inventory-resources" style="width:350px" name="redq_rental_off_days[]"
            data-placeholder="<?php esc_attr_e('Choose off days', 'woocommerce'); ?>"
            title="<?php esc_attr_e('Weekends', 'woocommerce') ?>" class="wc-enhanced-select">
            <?php if (is_array($days) && !empty($days)) : ?>
            <?php foreach ($days as $key => $value) { ?>
            <option value="<?php echo esc_attr($key); ?>" <?php if (in_array($key, $rental_off_days)) { ?> selected
                <?php
                                                                                                                        } ?>>
