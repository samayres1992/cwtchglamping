...
if (!function_exists('the_mounty_customizer_get_css')) {

	function the_mounty_customizer_get_css($args=array()) {
		...
		if ($fonts) {
			...
			$fonts = the_mounty_customizer_add_theme_fonts($fonts);
...
.post_meta_single .post_meta_item, 
...
.wp-block-calendar th, .wp-block-calendar td, .wp-block-calendar caption,
.sidebar .widget_product_categories li a, .sidebar .widget_categories li a, 
.sidebar .widget_meta li a, .sidebar .widget_archive li a, 
.sidebar .widget_product_categories li .wrap_in, .sidebar .widget_categories li .wrap_in, 
.sidebar .widget_meta li .wrap_in, .sidebar .widget_archive li .wrap_in, 
...
EOT;
			$rez = apply_filters('the_mounty_filter_get_css', $rez, array('fonts' => $fonts));
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ($vars) {
			...
			// Make theme-specific fonts rules
			$vars = the_mounty_customizer_add_theme_vars($vars);
			extract($vars);
...
EOT;
			$rez = apply_filters('the_mounty_filter_get_css', $rez, array('vars' => $vars));
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($scheme) ? array_keys(the_mounty_get_sorted_schemes()) : array($scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $s) {
					// Prepare colors
					if (empty($scheme)) $colors = the_mounty_get_scheme_colors($s);
...
.trx_addons_popup input[type="number"],
.trx_addons_popup input[type="email"],
.trx_addons_popup input[type="url"],
.trx_addons_popup input[type="tel"],
.trx_addons_popup input[type="search"],
...
.sidebar .widget .select_container select:focus {
...
.scheme_self.sidebar input[type="submit"]:not(.search-submit),
...
.scheme_self.sidebar .widget.woocommerce input[type="submit"]:not(.search-submit),
...
.scheme_self.sidebar input[type="submit"]:not(.search-submit):hover,
.scheme_self.sidebar input[type="submit"]:not(.search-submit):focus,
...
header.scheme_dark .sc_layouts_iconed_text_link:hover .sc_layouts_item_icon,
header.scheme_dark .sc_layouts_iconed_text_link:hover span{
	color: {$colors['text_hover']};
}
...
.the_mounty_tabs .the_mounty_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.the_mounty_tabs .the_mounty_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .the_mounty_tabs .the_mounty_tabs_titles li a {
	color: {$colors['alter_dark']};
	background-color: {$colors['alter_bg_hover']};
}
.sheme_self.sidebar div.esg-filter-wrapper .esg-filterbutton > span:hover,
.sheme_self.sidebar .mptt-navigation-tabs li a:hover,
.sheme_self.sidebar .the_mounty_tabs .the_mounty_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['alter_link']};
}
...
.sheme_self.sidebar .the_mounty_tabs .the_mounty_tabs_titles li.ui-state-active a {
...
.post_item_single .post_content .post_tags.post_meta_item,
...
.wp-block-calendar tbody td#today a:hover {
	color: {$colors['text_dark']};
}
...
.scheme_self.sidebar .widget_product_categories li {
	color: {$colors['alter_dark']};
}
...
.scheme_self.sidebar .sc_edd_details .downloads_page_tags .downloads_page_data > a,
.scheme_self.sidebar .widget_product_tag_cloud a, .scheme_self.sidebar .widget_tag_cloud a,
...
EOT;
					...
					$rez = apply_filters('the_mounty_filter_get_css', $rez, array('colors' => $colors, 'scheme' => $s));
		...
		return apply_filters( 'the_mounty_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>