...
------------------------------------------------------------------------------- */
if (!function_exists('the_mounty_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'the_mounty_trx_addons_theme_specific_components');
	function the_mounty_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('the_mounty_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'the_mounty_trx_addons_theme_specific_setup1', 1 );
	function the_mounty_trx_addons_theme_specific_setup1() {
		if (the_mounty_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'the_mounty_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'the_mounty_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'the_mounty_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'the_mounty_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'the_mounty_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'the_mounty_trx_addons_widgets_list');

			// Special filter 'trx_addons_filter_sc_item_link_classes'
			add_filter( 'trx_addons_filter_sc_item_link_classes', function($args){
			    return str_replace('sc_button_size_small', 'sc_button_size_medium', $args);
            });
            // Properties without single page style "Tabs". Only "Default"
            add_filter( 'trx_addons_filter_cpt_single_styles', function(){
			    return array('default' => esc_html__('Default', 'the-mounty'));
            });
		}
	}
}
...
// CV
if ( !function_exists( 'the_mounty_trx_addons_cv_enable' ) ) {
	
	function the_mounty_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return THE_MOUNTY_THEME_FREE
					? false		// Free version
					: true;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'the_mounty_trx_addons_demo_enable' ) ) {
	
	function the_mounty_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'the_mounty_trx_addons_edd_themes_market_enable' ) ) {
	
	function the_mounty_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'the_mounty_trx_addons_api_list' ) ) {
	
	function the_mounty_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'the_mounty_trx_addons_cpt_list' ) ) {
	
	function the_mounty_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'the_mounty_trx_addons_sc_list' ) ) {
	
	function the_mounty_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'the_mounty_trx_addons_widgets_list' ) ) {
	
	function the_mounty_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'the_mounty_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'the_mounty_trx_addons_menu_cache');
	function the_mounty_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('the_mounty_trx_addons_localize_script')) {
	...
	function the_mounty_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = the_mounty_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'the_mounty_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'the_mounty_trx_addons_sc_type', 10, 2);
	function the_mounty_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ($sc == 'trx_sc_testimonials') {
			$list['default light'] = esc_html__('Default light', 'the-mounty');
		}
		if ($sc == 'trx_sc_blogger') {
			$list['default transparent'] = '/wp-content/plugins/trx_addons/components/shortcodes/blogger/type-default.png';
		}
		if ($sc == 'trx_sc_blogger') {
			$list['products'] = '/wp-content/plugins/trx_addons/components/shortcodes/blogger/type-plain.png';
		}
		return $list;
	}
}
...
if ( !function_exists( 'the_mounty_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'the_mounty_trx_addons_sc_atts', 10, 2);
	function the_mounty_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'the_mounty_trx_addons_video_mask_output' ) ) {
    add_filter('trx_addons_filter_video_mask', 'the_mounty_trx_addons_video_mask_output', 10, 2);
    function the_mounty_trx_addons_video_mask_output($output, $args)
    {
        if (!empty($args['cover_title']) && !empty($args['cover_subtitle']) && !empty($output)) {
            $output = '<div class="cover_video_mask video_mask"></div>'
                . '<div class="cover_wrap">'
                    . (!empty($args['cover_subtitle']) ? '<div class="cover_text cover_subtitle">' . the_mounty_prepare_macros($args['cover_subtitle']) . '</div>' : '')
                    . (!empty($args['cover_title']) ? '<div class="cover_text cover_title">' . the_mounty_prepare_macros($args['cover_title']) . '</div>' : '')
                    . ($args['popup']
                        ? '<a class="cover_button video_hover trx_addons_popup_link" href="#' . esc_attr($args['id']) . '_popup"></a>'
                        : '<div class="cover_button video_hover" data-video="' . esc_attr($args['embed']) . '"></div>'
                    )
                . '</div>';
        }
        return $output;
...
if ( !function_exists( 'the_mounty_trx_addons_video_cover_thumb_size' ) ) {
    add_filter('trx_addons_filter_video_cover_thumb_size', 'the_mounty_trx_addons_video_cover_thumb_size');
    function the_mounty_trx_addons_video_cover_thumb_size()
    {
        return 'full';
    }
}
...
if ( !function_exists( 'the_mounty_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'the_mounty_trx_addons_sc_map', 10, 2);
	function the_mounty_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'the-mounty'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'the-mounty') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(the_mounty_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'the_mounty_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'the_mounty_trx_addons_sow_map', 10, 2);
	function the_mounty_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'the-mounty'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'the-mounty') ),
				'options' => the_mounty_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			the_mounty_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			the_mounty_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'the_mounty_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'the_mounty_trx_addons_sc_output', 10, 4);
	function the_mounty_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !the_mounty_is_inherit($atts['color_style']))
...
if ( !function_exists( 'the_mounty_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'the_mounty_trx_addons_sc_item_title_tag');
	function the_mounty_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'the_mounty_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'the_mounty_trx_addons_sc_item_button_args', 10, 3);
	function the_mounty_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'the_mounty_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'the_mounty_trx_addons_slider_title', 10, 2 );
	function the_mounty_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title']))
...
if ( !function_exists( 'the_mounty_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'the_mounty_trx_addons_sc_googlemap_styles');
	function the_mounty_trx_addons_sc_googlemap_styles($list) {
		$list['light'] = esc_html__('Light', 'the-mounty');
		return $list;
	}
}
...
if ( !function_exists( 'the_mounty_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'the_mounty_trx_addons_tiny_mce_style_formats');
	function the_mounty_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'the-mounty') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'the-mounty'),
								'selector' => 'ul',
...
if ( !function_exists( 'the_mounty_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'the_mounty_filter_allow_override_header_image', 'the_mounty_trx_addons_allow_override_header_image' );
	function the_mounty_trx_addons_allow_override_header_image($allow) {
		return the_mounty_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'the_mounty_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'the_mounty_trx_addons_thumb_size', 10, 2);
	function the_mounty_trx_addons_thumb_size($thumb_size='', $type='') {
...
if (!function_exists('the_mounty_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'the_mounty_trx_addons_override_options_fields', 10, 2);
	function the_mounty_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'the-mounty'),
				"desc" => wp_kses_data( __("Team member's email", 'the-mounty') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
		...
		if (defined('TRX_ADDONS_CPT_PROPERTIES_PT') && $post_type==TRX_ADDONS_CPT_PROPERTIES_PT) {
            $mb['new_section'] = array(
                "title" => esc_html__('Additional params', 'the-mounty'),
                "desc" => '',
                "priority" => 200,
                "type" => "section",
            );
			$mb['hot'] = array(
				"title" => esc_html__("Hot offer",  'the-mounty'),
				"desc" => wp_kses_data( __("Display label \"HOT\" in picture", 'the-mounty') ),
                "std" => 0,
                "details" => true,
                "type" => "checkbox"
			);
            $mb['button_info'] = array(
                "title" => esc_html__('Button for Order', 'the-mounty'),
                "desc" => 'The button for the order of Property. Works only with the included plugin "Booked"',
                "type" => "info",
            );
            if ( class_exists('booked_plugin') ) {
                $mb['button_book_text'] = array(
                    "title" => esc_html__("Button Text", 'the-mounty'),
                    "desc" => wp_kses_data(__("Text for Button", 'the-mounty')),
                    "details" => true,
                    "std" => "Book a Property",
                    "type" => "text"
                );
                $mb['button_book_link'] = array(
                    "title" => esc_html__("Button Link", 'the-mounty'),
                    "desc" => wp_kses_data(__("Link for Button", 'the-mounty')),
                    "details" => true,
                    "std" => "",
                    "type" => "text"
                );
