...
if (!function_exists('the_mounty_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'the_mounty_hovers_theme_setup3', 3 );
	function the_mounty_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		the_mounty_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'the-mounty'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'the-mounty') ),
				"std" => 'slide_left',
				"options" => array(
					'slide_left'	=> esc_html__('Slide from Left',	'the-mounty'),
					'slide_right'	=> esc_html__('Slide from Right',	'the-mounty'),
					'slide_top'		=> esc_html__('Slide from Top',		'the-mounty'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'the-mounty'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'the-mounty'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'the-mounty') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'the-mounty'),
					'icon'	=> esc_html__('Icon',	'the-mounty')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('the_mounty_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'the_mounty_hovers_theme_setup9', 9 );
	function the_mounty_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'the_mounty_hovers_frontend_scripts', 1010 );
		add_filter( 'the_mounty_filter_localize_script','the_mounty_hovers_localize_script' );
		add_filter( 'the_mounty_filter_merge_scripts',	'the_mounty_hovers_merge_scripts' );
		add_filter( 'the_mounty_filter_merge_styles',	'the_mounty_hovers_merge_styles' );
		add_filter( 'the_mounty_filter_merge_styles_responsive', 'the_mounty_hovers_merge_styles_responsive' );
		add_filter( 'the_mounty_filter_get_css', 		'the_mounty_hovers_get_css', 10, 2 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'the_mounty_hovers_frontend_scripts' ) ) {
	
	function the_mounty_hovers_frontend_scripts() {
		if ( the_mounty_is_on(the_mounty_get_theme_option('debug_mode')) && the_mounty_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'the-mounty-hovers', the_mounty_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('the_mounty_hovers_merge_styles')) {
	
	function the_mounty_hovers_merge_styles($list) {
...
if (!function_exists('the_mounty_hovers_merge_styles_responsive')) {
	
	function the_mounty_hovers_merge_styles_responsive($list) {
...
if (!function_exists('the_mounty_hovers_localize_script')) {
	
	function the_mounty_hovers_localize_script($arr) {
		$arr['button_hover'] = the_mounty_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('the_mounty_hovers_merge_scripts')) {
	
	function the_mounty_hovers_merge_scripts($list) {
...
if ( !function_exists('the_mounty_hovers_add_icons') ) {
	function the_mounty_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					the_mounty_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'the-mounty') : the_mounty_woocommerce_add_to_cart_text()) : '')
										. '</a>',
										$product));
				}
                 ?>
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php echo !empty($post_link) ? esc_url($post_link) : esc_attr($no_link); ?>" aria-hidden="true" class="icon-015_link"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'the_mounty_hovers_get_css' ) ) {
	
	function the_mounty_hovers_get_css($css, $args) {
...
a.vc_btn3 {	background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
...
.sc_button_hover_style_inverse.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['inverse_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['inverse_link']} !important; }
footer .sc_button_hover_style_inverse.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_right {	background: linear-gradient(to left,	{$colors['text_hover']} 50%, {$colors['inverse_link']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['inverse_link']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_hover']} 50%, {$colors['inverse_link']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['inverse_link']} !important; }
.sc_button_hover_style_inverse.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_hover']} 50%, {$colors['inverse_link']} 50%) no-repeat scroll right top / 100% 210% {$colors['inverse_link']} !important; }
...
.sc_button_hover_style_alterbd.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_right {	background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_bg_color']} !important; }
...
.sc_button_hover_style_alter.sc_button_hover_slide_bottom:hover  {	color: {$colors['inverse_text']} !important; }
...
footer .sc_button_hover_slide_left:hover,
.sc_button_hover_slide_left.active,
footer .sc_button_hover_slide_left.active,
.ui-state-active .sc_button_hover_slide_left,
.vc_active .sc_button_hover_slide_left,
.vc_tta-accordion .vc_tta-panel-title:hover .sc_button_hover_slide_left,
li.active .sc_button_hover_slide_left,
a.vc_btn3:hover { background-position: left bottom !important; color: {$colors['inverse_text']} !important; }
