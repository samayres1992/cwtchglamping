<div class="front_page_section front_page_section_features<?php
			$the_mounty_scheme = the_mounty_get_theme_option('front_page_features_scheme');
			if (!the_mounty_is_inherit($the_mounty_scheme)) echo ' scheme_'.esc_attr($the_mounty_scheme);
			echo ' front_page_section_paddings_'.esc_attr(the_mounty_get_theme_option('front_page_features_paddings'));
		?>"<?php
		$the_mounty_css = '';
		$the_mounty_bg_image = the_mounty_get_theme_option('front_page_features_bg_image');
...
?>><?php
	// Add anchor
	$the_mounty_anchor_icon = the_mounty_get_theme_option('front_page_features_anchor_icon');	
	$the_mounty_anchor_text = the_mounty_get_theme_option('front_page_features_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_features_inner<?php
			if (the_mounty_get_theme_option('front_page_features_fullheight'))
				echo ' the_mounty-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$the_mounty_css = '';
			$the_mounty_bg_mask = the_mounty_get_theme_option('front_page_features_bg_mask');
			$the_mounty_bg_color = the_mounty_get_theme_option('front_page_features_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$the_mounty_caption = the_mounty_get_theme_option('front_page_features_caption');
			if (!empty($the_mounty_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo !empty($the_mounty_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($the_mounty_caption, 'the_mounty_kses_content' ); ?></h2><?php
			}
		
			// Description (text)
			$the_mounty_description = the_mounty_get_theme_option('front_page_features_description');
			if (!empty($the_mounty_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo !empty($the_mounty_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($the_mounty_description), 'the_mounty_kses_content' ); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_features_output"><?php 
				if (is_active_sidebar('front_page_features_widgets')) {
					...
					if (!the_mounty_exists_trx_addons())
						...
						the_mounty_customizer_need_widgets_message('front_page_features_caption', 'ThemeREX Addons - Services');
				}
			?></div>
		</div>
	</div>
