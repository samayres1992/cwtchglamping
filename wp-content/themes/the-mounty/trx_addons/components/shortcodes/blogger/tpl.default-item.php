...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) . (empty($post_link) ? ' no_links' : '') ); ?>><?php
    ...
    if (has_post_thumbnail( get_the_id() )) {
        ...
        // Post meta
        $the_mounty_show_date_in_featured = in_array($post_format, array('standard'));
        if ($the_mounty_show_date_in_featured) { ?>
		...
		if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) {
			?><div class="sc_blogger_item_excerpt">
				...
				if ( $show_more && !empty($post_link) && !empty($args['more_text']) ) {
					?><div class="sc_blogger_item_button sc_item_button"><a href="<?php echo esc_url($post_link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_size_medium', 'sc_blogger', $args)); ?>"><?php
