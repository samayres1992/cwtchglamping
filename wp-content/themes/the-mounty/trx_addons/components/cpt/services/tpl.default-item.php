...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			?>><?php
				if (!empty($svg)) {
					...
					?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e( 'Image', 'the-mounty' ); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			echo !empty($link) 
				? '</a>' 
				: '</span>';
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e( 'Image', 'the-mounty' ); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
		} else {
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			...
			} elseif (!empty($link) && empty($args['more_text'])) {
				?><a href="<?php echo esc_url($link); ?>" class="sc_services_item_button_link"></a><?php
			}
		}
	?></div>
</div>
